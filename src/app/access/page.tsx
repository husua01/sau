...
import { createDownloadableBlob, downloadFile } from "@/lib/file-encryption";
import { buildSiweMessage } from "@/lib/auth";
import { BrowserProvider, Contract } from "ethers";

const LEGACY_ENCRYPTION_UNSUPPORTED_MESSAGE =
  "이 콘텐츠는 더 이상 지원하지 않는 방식(web-crypto)으로 암호화되어 이 화면에서 복호화할 수 없습니다. " +
  "해당 방식은 복호화 키가 노출되는 보안 결함이 있어 지원이 중단되었으며, 콘텐츠는 이미 공개된 것으로 간주해야 합니다. " +
  "크리에이터는 콘텐츠를 다시 업로드/민팅해야 합니다.";
...
// 큰 파일의 암호문은 메타데이터에 인라인으로 들어있지 않고 Arweave URL
// (ciphertextUrl)로만 참조된다. 인라인이 없으면 그 URL에서 받아온다.
async function resolveCiphertext(encryptionData: any): Promise<string> {
  if (
    typeof encryptionData?.ciphertext === "string" &&
    encryptionData.ciphertext
  ) {
    return encryptionData.ciphertext;
  }
  if (
    typeof encryptionData?.ciphertextUrl === "string" &&
    encryptionData.ciphertextUrl
  ) {
    const res = await fetch(encryptionData.ciphertextUrl);
    if (!res.ok) {
      throw new Error(`암호문 다운로드 실패 (status ${res.status})`);
    }
    return await res.text();
  }
  throw new Error("암호문을 찾을 수 없습니다.");
}
...
export default function AccessDataPage() {
  ...
  // /api/unified 의 SIWE 보호 액션 호출용 헬퍼. auth.ts의 verifySiwe는 nonce를
  // 1회용으로 소비하므로(재사용 시 401) 매 호출마다 새 서명을 요청한다.
  // 서명은 어디에도 캐시하지 않고(특히 localStorage 금지) 즉시 사용 후 버린다.
  const signedFetch = useCallback(
    async (action: string, payload: Record<string, any>) => {
      ...
      // 전환 전에 저장된 state)을 그대로 쓰면 서명자와 userAddress가 어긋나 서버의
      // SIWE 검증이 매번 401로 실패한다.
  ...
  const fetchUserNFTs = async (address: string) => {
    ...
    try {
      ...
      if (!contractAddress) {
        throw new Error(
          "NEXT_PUBLIC_SAU_CONTRACT_ADDRESS가 설정되지 않았습니다.",
        );
      }

      console.log(
        ` NFT 조회 시작 (블록체인 원본) - 컨트랙트: ${contractAddress}, 사용자: ${address}`,
      );

      // 블록체인/서버에서 직접 조회
      const response = await signedFetch("get_user_nfts", {
        userAddress: address,
      ...
      if (normalizedNFTs.length === 0) {
        console.warn(
          " 온체인 NFT를 찾지 못했습니다. 연결 상태와 컨트랙트 주소를 확인하세요.",
        );
      }
  ...
  // 지갑 계정 전환 감지. walletAddress를 갱신하지 않으면 signedFetch가 새 서명자와
  // 다른(오래된) userAddress를 보내 모든 요청이 401로 실패하고, balanceOf/burn 등도
  // 잘못된 주소를 대상으로 실행된다.
  useEffect(() => {
    const ethereum = (window as any).ethereum;
    ...
    const handleAccountsChanged = (accounts: string[]) => {
      // 소각 확인 모달이 이전 계정의 NFT를 대상으로 열려 있으면 새 계정으로 그대로
      // 진행되지 않도록 항상 닫는다. 다만 이미 서명/전송까지 들어간 소각 트랜잭션은
      // 계정 전환과 무관하게 그대로 진행되므로("signing"/"pending"), 그 진행 상태
      // 표시까지 여기서 지우면 실제로는 계속 진행 중인데 아무 일도 없었던 것처럼
      // 보이게 된다. 그 경우는 performBurn이 스스로 success/error로 정리하도록 둔다.
      setShowBurnConfirm(false);
      setConfirmBurnTarget(null);
      setBurnStatus((prev) =>
        prev.step === "signing" || prev.step === "pending"
          ? prev
          : { step: "idle" },
      );
  ...
  const selectNFT = async (nft: any) => {
    ...
    try {
      ...
      // 1. NFT 소유권 확인
      const ownershipResponse = await signedFetch("check_nft_ownership", {
      ...
      const metadataResponse = await signedFetch("get_nft_metadata", {
        contractAddress: nft.contractAddress,
        tokenId: nft.tokenId,
        userAddress: walletAddress,
      });
      ...
      if (
        ...
        const isTextContent =
          (properties.encrypted !== undefined
  ...
  function resolvePreviewUrl(
    resultData: any,
    variant?: "image" | "video" | "audio" | "text" | "pdf" | "default",
  ): string | null {
  ...
  const checkNFTOwnership = async (
    ...
    try {
      ...
      const response = await signedFetch("check_nft_ownership", {
        contractAddress,
        tokenId,
        userAddress: walletAddress,
      });
  ...
  const decryptAndShowText = async (encryptionData: any, fileName: string) => {
    ...
    try {
      ...
      if (
        encryptionData.encryptionType === "lit-protocol" ||
        encryptionData.ciphertext ||
        encryptionData.ciphertextUrl
      ) {
        try {
          const { initLitClient, decryptWithLit } =
            await import("@/lib/lit-protocol");
          const litChain =
          ...
          const litSigner = await new BrowserProvider(
            (window as any).ethereum,
          ).getSigner();

          const ciphertext = await resolveCiphertext(encryptionData);
          ...
          console.warn(" Lit Protocol 텍스트 복호화 실패:", litError);
        }
      }
      ...
      if (!decryptedContent) {
        if (
          ...
          throw new Error(LEGACY_ENCRYPTION_UNSUPPORTED_MESSAGE);
        }
  ...
  useEffect(() => {
    if (result && result.success && result.encryptionData && !decryptingFile) {
      const key =
  ...
  const decryptEncryptedFile = async (
    ...
    try {
      ...
      if (
        actualEncryptionData.encryptionType === "lit-protocol" ||
        actualEncryptionData.ciphertext ||
        actualEncryptionData.ciphertextUrl
      ) {
        ...
        try {
          ...
          const ciphertext = await resolveCiphertext(actualEncryptionData);
          ...
          if (result) {
            ...
            const isBinaryContent =
              ...
              (actualEncryptionData.mimeType &&
                !actualEncryptionData.mimeType.startsWith("text/"));

            if (isBinaryContent) {
              decryptedBytes = base64ToUint8Array(result);
              console.log(
                " Lit 복호화 결과 (base64 bytes):",
          ...
          console.warn(" Lit Protocol 복호화 실패:", litError);
          throw litError;
        }
      }
      ...
      if (decryptedBytes && decryptedBytes.length > 0) {
        const mimeType =
          actualEncryptionData.mimeType ||
          (actualEncryptionData.encoding === "utf-8"
            ? "text/plain"
        ...
        setResult((prev: any) => ({
          ...
          decryptedPreview: previewUrl
            ? { url: previewUrl, mimeType, fileName: safeFileName, blob }
            : null,
          decryptedContent: isTextMime ? decryptedTextContent : null,
        }));
      } else if (
        actualEncryptionData.encryptionType &&
        actualEncryptionData.encryptionType !== "lit-protocol"
      ) {
        throw new Error(LEGACY_ENCRYPTION_UNSUPPORTED_MESSAGE);
      } else {
  ...
  const performBurn = useCallback(
    async (nft: any) => {
      ...
      setBurnStatus({
        step: "validating",
        message: "지갑 주소 및 네트워크를 확인하는 중입니다.",
      });
      setBurningNFT(true);
      ...
      try {
        ...
        if (signerAddress !== expectedAddress) {
          throw new Error(
            `연결된 지갑 주소(${signerAddress})와 화면에 표시된 주소(${expectedAddress})가 일치하지 않습니다.`,
          );
        }
        ...
        setBurnStatus({
          step: "signing",
          message: "MetaMask에서 파기 트랜잭션 서명 요청을 확인해주세요.",
        });
        ...
        const gasEstimate = await contract.burn.estimateGas(
          walletAddress,
          nft.tokenId,
          1,
        );
          ...
          >
            ...
            {!walletConnected ? (
              <div style={{ textAlign: "center" }}>
                ...
                >
                  NFT 소유권을 확인하고 데이터에 접근하기 위해 MetaMask를
                  연결해주세요
                </p>
                <button
                  ...
                  >
                    ...
                    >
                      {userNFTs.map((nft, index) => {
                        ...
                        const displayTokenId = shortenDisplayText(
                          nft.tokenId,
                          8,
                        );
                    ...
                    {result.encryptionData &&
                      (() => {
                        ...
                        const rawMimeLabel =
                          ...
                          result.metadata?.properties?.mimeType ||
                          "application/octet-stream";
                        const displayFileName = shortenDisplayText(
                          fileDisplayName,
                          48,
                        );
                        const mimeLabel = rawMimeLabel;
                        const displayMimeLabel = shortenDisplayText(
                          mimeLabel,
                          48,
                        );
                        const sizeLabel = formatBytes(
                          result.encryptionData?.fileMetadata?.size,
                        );
                            ...
                            >
                              ...
                              >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  >
                                    {contentInfo.label} · {displayMimeLabel}
                                    {sizeLabel ? ` · ${sizeLabel}` : ""}
                                  </p>
                                  <p
                                    style={{
