...
export async function uploadImageToPinata(
  ...
  try {
    console.log(
      `📤 Pinata IPFS 업로드 시작: ${file.name} (${file.size} bytes)`,
    );
    ...
    if (!pinataApiKey || !pinataSecretKey) {
      ...
      console.log(
        "💡 .env.local에 PINATA_API_KEY와 PINATA_SECRET_KEY를 추가하세요",
      );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Pinata API 오류: ${response.status} - ${errorData.error?.details || errorData.error?.message || "Unknown error"}`,
      );
    }
